...
public class asset_rentals {
   ...
   public asset_rentals () {}; // Constructor of the DAO
   ...
   public int addRecord () { // Method add a Record
       try {
           ...
           PreparedStatement pstmt = conn.prepareStatement("INSERT INTO asset_rentals VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
           ...
           if ((accept_hoid == 0 ||
               ...
               pstmt = conn.prepareStatement("INSERT INTO asset_rentals VALUES (?,?,?,?,?,?,?,?,?,?,?,?,null)");
                     ...
                     "".equals(accept_electiondate)) &&
                    "".equals(return_date)) {
               pstmt = conn.prepareStatement("INSERT INTO asset_rentals VALUES (?,?,?,?,?,?,?,?,?,null,null,null,null)");
           }
   ...
   public int retRecord () { // Method dispose a Record
       try {
           ...
           System.out.println("Connection Successful");
           // 4. Prepare our INSERT Statement
           PreparedStatement pstmt = conn.prepareStatement("UPDATE asset_rentals SET return_date = ? WHERE asset_id = ? AND rental_date = ?");
           // 5. Supply the statement with values
           int x = 0;
           if (return_date == "") {
               pstmt = conn.prepareStatement("UPDATE asset_rentals SET return_date = null WHERE asset_id = ? AND rental_date = ?");
               x = 1;
           }
           else {
   ...
   public int delRecord () { // Method mark a Record as deleted
       try {
           ...
           System.out.println("Connection Successful");
           // 4. Prepare our INSERT Statement
           PreparedStatement pstmt = conn.prepareStatement("UPDATE asset_rentals SET status = \'C\', inspection_details = \'DELETED\'" +
